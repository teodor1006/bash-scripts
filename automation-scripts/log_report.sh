...
count_errors=$(grep -ciE "Error|failed" "$logfile")
...
top_errors=$(grep -iE "error|failed" "$log_file" | sort | uniq -c | sort -nr | head -n 5)
# Display the results 
...
if [ "$choice" == "y" ] || [ "$choice" == "Y" ]; then
    ...
    archive_name=$(basename "$log_file").$(date +"%Y%m%d_%H%M%S").tar.gz
